...
class Main(object):
	...
	def update(self):
		...
		print '%s %s updating...' % (t.now(), 'apt_cache_action.py')
		...
		try:
			...
			print '%s %s ERROR Connectivity issue while updating...' % (t.now(), 'apt_cache_action.py')

			return '%s %s connectivty issues' % (t.now(), 'apt_cache_action.py')
